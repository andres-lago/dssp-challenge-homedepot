...
from pyspark.ml.xgboost import XGBoost
...
data=sqlContext.read.format("com.databricks.spark.csv").\
    ...
    load(trainFileAbsPath).repartition(100)
...
attributes=sqlContext.read.format("com.databricks.spark.csv").\
	option("header", "true").\
	option("inferSchema", "true").\
	load("file://" + cwd + "../data_light/train.csv").repartition(100)
...
#create NEW features & train and evaluate model
...
# Fit the model
xgbModel = xgb.fit(sqlContext.createDataFrame(train))

#Apply model to test data
result=xgbModel.transform(sqlContext.createDataFrame(test))
